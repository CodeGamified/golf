...
{
    /// <summary>
    /// Procedural golf ball trail — same pattern as Pool.
    ///  • Low/Med/High: ring buffer of spheres that fade out.
    ///  • Ultra: persistent LineRenderer that draws the ball path.
    /// Golf uses XZ plane, Y = 0.1f (just above ground).
    /// </summary>
    public class GolfBallTrail : MonoBehaviour, IQualityResponsive
    {
        private int _trailLength;
        private const float TRAIL_INTERVAL = 0.02f;
        private const int ULTRA_THRESHOLD = 1000;
        private const float BALL_Y = 0.1f;
        ...
        private static readonly Color DefaultTrailHDR = new Color(3f, 3f, 3f);
        ...
        public void Initialize(GolfBall ball, Color trailColor)
        {
            _ball = ball;
        ...
        {
            var pos = new Vector3(_ball.Position.x, BALL_Y + 0.01f, _ball.Position.y);
        ...
        /// <summary>Set the trail color from this point forward.</summary>
        ...
        /// <summary>Clear the persistent line trail.</summary>
        ...
        private void SpawnSpherePoint()
        {
            ...
            part.position = new Vector3(_ball.Position.x, BALL_Y, _ball.Position.y);
            part.gameObject.SetActive(true);

            float speed = _ball.Velocity.magnitude;
            float s = Mathf.Lerp(0.04f, 0.1f, Mathf.Clamp01(speed / 3f));
